    ...
    <div>
        ...
        <b-container fluid class="mt-5">
            ...
            <b-table
                ...
                striped
                @filtered="onFiltered"
                empty-text="Nenhum transferencia para mostrar"
            >
                    ...
                    <b-card>
                        <ul>
                            <li
                                ...
                                :key="key"
                                v-if="['pais', 'garantia'].includes(key) && value !== null && value !== ''"
                            >
            ...
            <!-- Info modal -->
            <b-modal :id="`edit_transferencia_${infoModal.id}`" title="Editar" ref="modal" size="xl">
                <div v-if="selectedItem !== null">
                    <transferencia
                        :transferencia="selectedItem"
                        :dados="dados"
                        :garantias="garantias"
                    ></transferencia>
                </div>
...
export default({
    ...
    data () {
        return {
            ...
            fields: [
                ...
                { key: 'organizacao', label: 'Nome da Organização', sortable: true, class: 'text-center', stickyColumn: true },
    ...
    mounted () {
        ...
        this.fetchDados()
        this.fetchGarantias()
        this.$root.$on('transferencia:created', () => this.fetch())
        this.$root.$on('transferencia:updated', () => {
            this.resetInfoModal()
            this.fetch()
        })
        this.$root.$on('transferencia:removed', () => this.fetch())
    ...
    methods: {
        ...
        async fetchGarantias () {
            this.startFetching()
            await axios.get(`/transferencias/garantias`)
                .then(response => {
                    this.garantias = response.data.map((garantia, index) => {
                        return {
                            value: index,
                            text: garantia
                        }
        ...
        async fetch () {
            this.startFetching()
            await axios.post(`/processos/${this.processo.id}/transferencias`, {
                ...
                .then(response => {
                    ...
                    this.transferencias = this.paginator.data
        ...
        info(item, index, button) {
            ...
            this.$root.$emit('bv::show::modal', `edit_transferencia_${this.infoModal.id}`, button)
        },
        resetInfoModal() {
        ...
        save() {
            this.$root.$emit('transferencia:save')
        },
        async remove(transferencia_id) {
            this.startFetching()
            await axios.delete(`/transferencias/${transferencia_id}`)
                .then(response => {
                    this.$root.$emit('transferencia:removed')
                })
                .catch(error => {
                    console.log(error)
                })
        ...
        columnName(c)
        {
            if (c === 'pais') return 'País'
            if (c === 'garantia') return 'Garantia'
        }
    },
    computed: {
        sortOptions() {
