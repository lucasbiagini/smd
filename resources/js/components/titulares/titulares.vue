    ...
    <div>
        ...
        <b-container fluid class="mt-5">
            ...
            <b-table
                ...
                striped
                @filtered="onFiltered"
                empty-text="Nenhum titular para mostrar"
            >
...
export default({
    ...
    mounted () {
        this.fetch()
        this.fetchTiposSGD()
        this.$root.$on('titular:created', () => this.fetch())
        this.$root.$on('titular:updated', () => {
            this.resetInfoModal()
            this.fetch()
        })
        this.$root.$on('titular:removed', () => this.fetch())
    },
    methods: {
        startFetching () {
        ...
        async fetchTiposSGD () {
            this.startFetching()
            await axios.get(`/titulares/tipos`)
                .then(response => {
        ...
        async fetch () {
            this.startFetching()
            await axios.post(`/processos/${this.processo.id}/titulares`, {
                ...
                .then(response => {
                    ...
                    this.titulares = this.paginator.data
        ...
        save() {
            this.$root.$emit('titular:save')
        },
        async remove(titular_id) {
            this.startFetching()
            await axios.delete(`/titulares/${titular_id}`)
                .then(response => {
                    this.$root.$emit('titular:removed')
                })
                .catch(error => {
                    console.log(error)
                })
